...
def home():
    ...
    upcoming_interviews = q.filter(Customer.interview_date >= now).order_by(Customer.interview_date).limit(5).all()
...
def add_interview(customer_id):
    ...
    db.session.commit()
    log_action('add_interview', f'مقابلة مع: {customer.full_name}', customer_id, 'customer')
    flash('تم إضافة المقابلة', 'success')
    return redirect(url_for('sales.customer_profile', customer_id=customer_id))
