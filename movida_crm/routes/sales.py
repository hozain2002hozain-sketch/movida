...
def home():
    ...
    no_answer = na_q.order_by(NoAnswerNumber.added_at.desc()).all()
...
def customers_list():
    ...
    followup_date = request.args.get('followup_date', '').strip()
    ...
    if followup_date:
        try:
            target_date = datetime.strptime(followup_date, '%Y-%m-%d')
            next_date = target_date + timedelta(days=1)
            # Filter customers with followups on this specific date
            query = query.filter(
                Customer.followup_date >= target_date,
                Customer.followup_date < next_date
            )
        except ValueError:
            pass
