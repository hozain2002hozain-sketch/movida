...
def home():
    ...
    # التحقق من وجود رقم موجود في session
    existing_number = session.pop('existing_number', None)
    show_existing_modal = request.args.get('show_existing_modal', 'false') == 'true'
    
    return render_template('social/home.html', numbers=numbers, total=total, sent=sent, remaining=remaining, sales_employees=sales_employees, existing_number=existing_number, show_existing_modal=show_existing_modal)
...
def add_number():
    if request.method == 'POST':
        ...
        if existing:
            ...
            flash(f'⚠️ الرقم {phone} موجود بالفعل في النظام وموجود مع: {sales_name}', 'warning')
...
def performance():
    ...
    for emp in sales_employees:
        ...
        from_other = Customer.query.filter(
            Customer.sales_employee_id == emp.id,
