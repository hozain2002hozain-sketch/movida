...
def home():
    ...
    # آخر المقابلات التي أضافها موظف الملفات الحالي فقط
    recent_interviews = Interview.query.filter_by(
        employee_id=current_user.id
    ).order_by(Interview.created_at.desc()).limit(5).all()
    
    # أقرب مواعيد المقابلات من Interview + Customer
    upcoming_interviews_files = Interview.query.filter(
        Interview.interview_date >= now
    ).order_by(Interview.interview_date).all()
    
    upcoming_interviews_customers = Customer.query.filter(
        Customer.interview_date >= now
    ).order_by(Customer.interview_date).all()
    
    # دمج وترتيب المقابلات من المصدرين معاً
    upcoming_interviews = sorted(
        list(upcoming_interviews_files) + list(upcoming_interviews_customers),
...
def add_interview():
    ...
    if request.method == 'POST':
        ...
        log_action('add_interview', f'مقابلة ملفات: {name} - {phone}', iv.id, 'interview')
        flash('تم إضافة المقابلة بنجاح', 'success')
        return redirect(url_for('files.home'))
    return render_template('files/add_interview.html', sales_users=sales_users)
...
def add_contract():
    ...
    if request.method == 'POST':
        ...
        if not sales_id:
            ...
            return render_template('files/add_contract.html', sales_users=sales_users)
        sales_name = ''
        sales_user = None
        try:
            if sales_id:
...
def interviews_list():
    ...
    # جلب المقابلات من موظفي المبيعات (Customer)
    sales_interviews = Customer.query.filter(
        Customer.interview_date.isnot(None)
    ).order_by(Customer.interview_date.desc()).all()
    
    # جلب المقابلات من ملفات
    files_interviews = Interview.query.filter(
        Interview.interview_date.isnot(None)
    ).order_by(Interview.interview_date.desc()).all()
    
    # دمج المقابلات
    all_interviews = list(sales_interviews) + list(files_interviews)
    all_interviews.sort(key=lambda x: x.interview_date, reverse=True)
